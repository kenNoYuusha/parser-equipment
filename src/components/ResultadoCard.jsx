...
const ResultadoCard = ({ resultado }) => {
  ...
  return (
    <div className="bg-bg-app animate-in fade-in duration-500">
      {/* Header con Serial */}
      <div className="bg-dark p-6 text-white flex justify-between items-center">
        ...
        {isRefurbished && (
          <div className="flex items-center space-x-2 bg-orange-500/20 border border-orange-500/40 px-4 py-2 rounded-xl">
            <span className="w-2 h-2 bg-orange-500 rounded-full animate-pulse"></span>
            <span className="text-orange-400 font-fjalla uppercase tracking-[0.2em] text-xs md:text-sm font-bold">
              Refurbished
            </span>
          </div>
        )}
      ...
      <div className="p-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            <div className="flex items-center space-x-3 border-b border-dark/10 pb-3">
              <div className="w-2 h-6 bg-primary rounded-full"></div>
              <h3 className="text-lg text-dark font-fjalla uppercase">Technical Specs</h3>
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center space-x-3 border-b border-dark/10 pb-3">
              <div className="w-2 h-6 bg-primary rounded-full"></div>
              <h3 className="text-lg text-dark font-fjalla uppercase">Commercial Info</h3>
            </div>

            {datosComerciales ? (
              <div className="bg-dark/5 p-5 rounded-2xl space-y-4 border border-dark/5 relative">
              ...
              <div className="h-full flex flex-col items-center justify-center p-8 bg-dark/5 border-2 border-dashed border-dark/10 rounded-2xl text-center">
                <span className="text-3xl mb-3 opacity-50">🔍</span>
                <p className="text-dark/40 text-sm italic">
                  No commercial information found for model <span className="text-dark font-bold">"{modelo}"</span>.
                </p>
              </div>
            )}
          </div>
...
const DataField = ({ label, value, subValue, highlight = false }) => (
  <div className="bg-dark/5 p-3 rounded-xl border border-dark/5">
