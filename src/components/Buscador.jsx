  ...
  if (cargando) {
    return (
      <div className="flex flex-col items-center justify-center min-h-100 space-y-4 font-lato">
        ...
        <p className="text-dark/70 font-medium">Synchronizing Databases...</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto px-4">
      ...
      {matchingKit && (
        <div className="mb-8 animate-in slide-in-from-top duration-500">
          <div className="bg-primary/10 border-2 border-primary/30 p-6 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-4 shadow-lg shadow-primary/5">
            ...
            {matchingKit.product_description && (
              <div className="text-right hidden md:block">
                <p className="text-dark/40 text-[10px] uppercase font-bold tracking-widest">Description</p>
                <p className="text-dark font-medium text-sm">{matchingKit.product_description}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-white/40 backdrop-blur-sm p-6 md:p-8 rounded-[2.5rem] border border-dark/5 shadow-inner mb-8">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 pb-6 border-b border-dark/5">
          <div className="text-left">
            <h3 className="text-sm text-dark/60 font-bold uppercase tracking-wider">Input Serials</h3>
            <p className="text-[10px] text-dark/40 uppercase tracking-widest">Max 10 units per batch</p>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 bg-dark text-white rounded-xl font-bold uppercase tracking-widest text-[10px] transition-all hover:bg-dark/90 disabled:opacity-20 shadow-md"
            >
            ...
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 bg-white border border-dark/10 text-dark/60 rounded-xl font-bold uppercase tracking-widest text-[10px] hover:bg-red-50 hover:text-red-500 transition-all"
            >
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-150 flex items-center justify-center p-4 animate-in fade-in duration-300">
          ...
          <div className="relative bg-white p-8 rounded-4xl shadow-2xl max-w-sm w-full text-center animate-in zoom-in-95 duration-300">
            <h3 className="text-xl font-fjalla text-dark uppercase mb-2">Clear Analysis?</h3>
            <p className="text-dark/50 text-sm font-lato mb-8">This will reset all inputs and results.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowResetConfirm(false)} className="flex-1 py-3 px-4 bg-dark/5 text-dark/60 rounded-xl font-bold uppercase tracking-widest text-[10px]">Cancel</button>
